...
/**
 * Base class for a Cloud Connector consuming OData feeds by performing read, create, update and delete operations.
...
public abstract class BaseODataConnector {
	...
	protected String getBaseServiceUri() {
		return this.baseServiceUri;
	}
	
	protected void setBaseServiceUri(String baseServiceUri) {
    ...
    public void createEntity(MuleMessage message,
		...
		if (!this.isBatchOperation(message, request.getRawRequest(getBaseServiceUri()))) {
			request.execute(getBaseServiceUri());
		}
    }
    ...
    public BatchResult batch(
    	...
    	OBatchRequest request = this.consumer.createBatch(getBaseServiceUri());
